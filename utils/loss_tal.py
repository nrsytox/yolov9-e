...
class ComputeLoss:
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        flat_feats = [t for sublist in p for t in sublist][-3:]

        batch_size = flat_feats[0].shape[0]

        #print([xi.shape for xi in flat_feats])  # debug opcional

        pred = torch.cat([xi.view(batch_size, self.no, -1) for xi in flat_feats], dim=2
        ).permute(0, 2, 1).contiguous()
        # [B, HW, C] - sem permute extra
        pred_distri, pred_scores = pred.split([self.reg_max * 4, self.no - self.reg_max * 4], dim=2)

        dtype = pred_scores.dtype
        batch_size, grid_size = pred_scores.shape[:2]
        imgsz = torch.tensor(flat_feats[0].shape[2:], device=self.device, dtype=dtype) * self.stride[0] # image size (h,w)
